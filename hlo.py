...
class YouTubeDownloader(ctk.CTk):
    def __init__(self):
        ...
        # --- FIX FOR WINDOW ICON (Toolbar/Title Bar) ---
        # Using iconphoto allows us to use a PNG instead of an ICO file
        try:
            if os.path.exists(logo_path):
                ...
                # Set as window icon (True applies it to all future windows too)
                self.iconphoto(True, self.icon_photo)
            else:
                print("Warning: logo.png not found for window icon.")
        except Exception as e:
            print(f"Error setting window icon: {e}")

        # --- [OPTIONAL] CHANGE WINDOW TITLE BAR COLOR (Windows 11) ---
        try:
            from ctypes import windll, byref, sizeof, c_int
            self.update() 
            hwnd = windll.user32.GetParent(self.winfo_id())
            # Color #E53E3E (Red) in hex reverse for Windows: 0x003E3EE5
            color = 0x003E3EE5 
            windll.dwmapi.DwmSetWindowAttribute(hwnd, 35, byref(c_int(color)), sizeof(c_int))
        except Exception:
            pass

        # --- IN-APP LOGO (Image inside the app) ---
        try:
             if os.path.exists(logo_path):
